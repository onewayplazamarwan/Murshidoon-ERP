...
def load_data(file, columns):
    if os.path.exists(file):
        try:
            ...
            # تأمين تحويل التواريخ لمنع خطأ TypeError
...
if selected == "الرئيسية":
    ...
    if not df.empty:
        today = date.today()
        # تأمين فلترة المتأخرات لتجنب NaT
    ...
    with tabs[0]:
        with st.form("inv_form"):
            ...
            with c2: due_d = st.date_input("تاريخ الاستحقاق"); ts = st.time_input("البداية"); te = st.time_input("النهاية")
    ...
    with tabs[1]:
        if not df.empty:
            ...
            with st.form("edit_f"):
                e_sn = st.selectbox("الطالب", std_df["الاسم"].tolist(), index=0)
                e_m, e_due = st.text_input("المادة", value=df.at[idx, "المادة"]), st.date_input("الاستحقاق", value=df.at[idx, "تاريخ_الاستحقاق"] if pd.notnull(df.at[idx, "تاريخ_الاستحقاق"]) else date.today())
    ...
    if not df.empty:
        ...
        c1.metric("إجمالي المبيعات", f"{int(doc_data['الإجمالي'].sum())} ر.ق")
