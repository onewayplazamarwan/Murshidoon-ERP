...
# أعمدة البيانات الموحدة لضمان ثبات السيستم
...
# دالة تحميل البيانات مع تأمين التواريخ والأرقام
def load_data(file, columns):
    if os.path.exists(file):
        try:
            ...
            # تحويل التواريخ بشكل آمن لمنع الـ TypeError
            if "تاريخ_الاستحقاق" in df.columns:
                df["تاريخ_الاستحقاق"] = pd.to_datetime(df["تاريخ_الاستحقاق"], errors='coerce').dt.date
            if "تاريخ_الفاتورة" in df.columns:
                df["تاريخ_الفاتورة"] = pd.to_datetime(df["تاريخ_الفاتورة"], errors='coerce').dt.date
            # تأمين الأرقام
...
with st.sidebar:
    ...
    if st.button("🚪 تسجيل الخروج", use_container_width=True):
        ...
        st.rerun()
    st.markdown("---")
    selected = option_menu("القائمة الرئيسية", ["الرئيسية", "الطلاب", "الفواتير والأقساط", "المصروفات", "حسابات المحاضرين", "الحضور والغياب", "⚙️ الإعدادات"], 
...
if selected == "الرئيسية":
    ...
    if not df.empty:
        today = date.today()
        # فلترة آمنة للمتأخرات لمنع الـ TypeError
        overdue_df = df[(df['المتبقي'] > 0) & (pd.notnull(df['تاريخ_الاستحقاق']))]
        overdue_df = overdue_df[overdue_df['تاريخ_الاستحقاق'] < today].copy()
        ...
        else: st.success("لا توجد فواتير متأخرة حالياً")
    ...
    with t3:
        if not std_df.empty:
            target = st.selectbox("كشف حساب الطالب", std_df["الاسم"].tolist())
            ...
            if not st_bills.empty:
                ...
                components.html(generate_invoice_html(summary, center_info, title="Statement of Account"), height=550)
    with t4:
        if not std_df.empty:
            del_n = st.selectbox("حذف طالب نهائياً", std_df["الاسم"].tolist())
            if st.button("❌ حذف الطالب وكل فواتيره"):
    ...
    tabs = st.tabs(["🆕 إصدار فاتورة", "✏️ تعديل شامل", "🖨️ معاينة", "🗑️ حذف"])
    with tabs[0]:
    ...
    with tabs[1]:
        if not df.empty:
            ...
            with st.form("edit_f"):
                ...
                e_m, e_due = st.text_input("المادة", value=df.at[idx, "المادة"]), st.date_input("تعديل الاستحقاق", value=df.at[idx, "تاريخ_الاستحقاق"] if pd.notnull(df.at[idx, "تاريخ_الاستحقاق"]) else date.today())
                e_sup = st.selectbox("تغيير المشرف", ["الدكتور / ابراهيم عوض الله", "محمد عدلي", "أخرى"])
                e_lec = st.selectbox("تغيير المحاضر", lec_df["الاسم"].tolist() if not lec_df.empty else ["---"])
                e_tot, e_pdv = st.number_input("الإجمالي الجديد", value=float(df.at[idx, "الإجمالي"])), st.number_input("المدفوع الجديد", value=float(df.at[idx, "المدفوع"]))
    ...
    with tabs[3]:
        if not df.empty:
            del_id = st.selectbox("حذف فاتورة رقم", df["ID"].tolist())
    ...
    with st.form("exp"):
        item, amt = st.text_input("بند المصرف"), st.number_input("المبلغ")
        if st.form_submit_button("حفظ"):
    ...
    st.header("👨‍🏫 حسابات الدكاترة والمحاضرين")
    ...
    if not df.empty:
        ...
        c2.metric("المحصل فعلياً", f"{int(doc_data['المدفوع'].sum())} ر.ق")
        c3.metric("متبقي ديون الطلاب", f"{int(doc_data['المتبقي'].sum())} ر.ق")
    ...
    if not df.empty:
        ...
        if debt > 0: st.error(f"🛑 مديونية متأخرة: {int(debt)} ر.ق")
    ...
    with t2:
        ...
        with st.form("add_u"):
            un, up, ur = st.text_input("اسم المستخدم"), st.text_input("كلمة المرور", type="password"), st.selectbox("الرتبة", ["Admin", "User"])
    ...
    with t4:
        ...
        with st.form("pass"):
            ...
            if st.form_submit_button("تحديث الباسورد"):
